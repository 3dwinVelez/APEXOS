...
async function servicesRoutes(fastify) {
  ...
  fastify.put("/services/orders/:id/items/:itemId", { schema: schemas.orderItemUpdateSchema, preHandler: requirePermission("services", "write") }, (request) => service.updateOrderItem(request.user?.tenant_id, request.user, request.params.id, request.params.itemId, request.body));
  fastify.delete("/services/orders/:id/items/:itemId", { preHandler: requirePermission("services", "write") }, (request) => service.deleteOrderItem(request.user?.tenant_id, request.user, request.params.id, request.params.itemId));
  fastify.patch("/services/orders/:id/items/:itemId/status", { schema: schemas.orderItemStatusSchema, preHandler: requirePermission("services", "write") }, (request) => service.transitionOrderItem(request.user?.tenant_id, request.user, request.params.id, request.params.itemId, request.body));
  ...
  fastify.patch("/services/orders/:id/execution", { schema: schemas.executionSchema, preHandler: requirePermission("services", "write") }, (request) => service.moveToExecution(request.user?.tenant_id, request.user, request.params.id, request.body));
