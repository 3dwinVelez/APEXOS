...
export default function AdvisorReports() {
  ...
  return <div className="apex-workspace-shell space-y-4">
    ...
    {data && <>
      ...
      <section className="apex-section-card overflow-x-auto p-4"><h2 className="mb-3 font-semibold">Detalle por período y asesor</h2><p className="mb-3 text-xs text-neutral-500">“Con cotización” incluye las que generaron pedido; no se suma a “Con pedido”. Sin resultado comercial significa visita finalizada sin pedido ni cotización vigentes, aunque tenga observaciones. El valor suma los pedidos no cancelados de esas visitas finalizadas, según la fecha de agenda.</p><table className="w-full text-left text-sm"><thead className="bg-paper"><tr>{["Período", "Asesor", "Visitas", "Realizadas", "Pendientes", "Con pedido", "Valor de pedidos", "Con cotización", "Solo cotización", "Sin resultado", "Efectividad"].map(label => <th key={label} className="whitespace-nowrap p-3">{label}</th>)}</tr></thead><tbody>{data.rows.map(row => <tr className="border-t border-line" key={`${row.period}/${row.advisor_id}`}>{[row.period, row.advisor, row.total, row.completed, row.pending, row.with_order, money(row.order_value), row.with_quotation, row.quotation_only, row.without_result, percentage(row)].map((value, index) => <td className="p-3" key={index}>{value}</td>)}</tr>)}</tbody></table></section>
    </>}
  </div>;
}
