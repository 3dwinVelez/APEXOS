...
export function CustomerCombobox({ customers: source, value, onChange, optional = true }: { customers: any[]; value: string | number; onChange: (value: string) => void; optional?: boolean }) {
  ...
  return <div ref={root} className="relative" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false); }}>
    <label className="text-sm font-medium" htmlFor="visit-customer-search">Cliente{optional ? " (opcional)" : ""}</label>
