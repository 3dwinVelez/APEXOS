...
export default function TreasuryPage() {
  ...
  return <div className="space-y-5">
    <header><h1 className="text-3xl font-semibold">Tesorería</h1><p className="mt-1 text-sm text-neutral-600">Bancos, recaudos de clientes, pagos a proveedores y trazabilidad contable.</p></header>
    <nav className="flex flex-wrap gap-2">{[["payments", "Registrar pago"], ["banks", "Maestro de bancos"], ["report", "Reporte de pagos"]].map(([value, label]) => <button key={value} onClick={() => setTab(value as typeof tab)} className={`h-10 rounded-md px-4 text-sm ${tab === value ? "bg-apex text-white" : "border border-line bg-white"}`}>{label}</button>)}<Link className="inline-flex h-10 items-center rounded-md border border-line bg-white px-4 text-sm" href="/dashboard/tesoreria/anticipos">Anticipos y cruces</Link></nav>
    ...
    {tab === "payments" ? <form className="space-y-4" onSubmit={submitPayment}>
      <section className="grid gap-3 rounded-md border border-line bg-white p-4 md:grid-cols-3 xl:grid-cols-6">
