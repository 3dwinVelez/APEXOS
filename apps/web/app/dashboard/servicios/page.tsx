...
import { ArrowLeft, ClipboardCheck, FileText, Plus, Search, Settings2, Wrench } from "lucide-react";
...
export default function ServicesPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 pb-28 md:pb-6">
      ...
      <section className="grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-5">
        {[["Pendientes", kpis.pending], ["En curso", kpis.in_progress], ["Cerradas", kpis.closed], ["No ejecutadas", kpis.not_executed], ["Total", kpis.total]].map(([label, value]) => (
          <div className="rounded-md border border-line bg-white p-3 shadow-sm sm:p-4" key={label}>
      ...
      <section className="rounded-md border border-line bg-white p-3 shadow-sm sm:p-4">
        <div className="mb-4 grid gap-3 md:flex md:flex-wrap md:items-center md:justify-between">
          ...
          <select className="h-12 w-full rounded-md border border-line bg-white px-3 text-base md:h-10 md:w-auto md:text-sm" value={status} onChange={(event) => setStatus(event.target.value)}>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {filtered.map((order) => (
            <Link className="min-h-36 rounded-md border border-line p-3 text-left transition active:scale-[0.99] hover:border-apex hover:bg-paper sm:p-4" href={`/dashboard/servicios/${order.id}`} key={order.id}>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-neutral-600">
                    ...
                    <span className="max-w-full truncate rounded-md bg-paper px-2 py-1">{order.reference?.code || "Sin ref."}</span>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-[1fr_auto] gap-2 border-t border-line bg-white/95 p-3 pb-[calc(env(safe-area-inset-bottom)+12px)] backdrop-blur md:hidden">
