...
export default function InitialInventoryLoadPage() {
  ...
  async function validate() {
    ...
    try {
      const result = await send("/api/v1/inventory/initial-load/validate") as Preview;
      setPreview(result);
      showToast({ tone: "success", title: "Plantilla validada", description: `${result.totals.lines} posiciones están listas para revisión.` });
    }
    catch (err) {
      const detail = err instanceof Error ? err.message : "No se pudo validar la plantilla";
      setPreview(null); setError(detail);
      showToast({ tone: "error", title: "No se pudo validar la plantilla", description: detail, retry: () => void validate() });
    }
  ...
  async function post() {
    ...
    try {
      const result = await send("/api/v1/inventory/initial-load") as Posted;
      setPosted(result); setPreview(null); setFile(null);
      showToast({ tone: "success", title: `Cargue contabilizado — ${result.accounting_document.full_number}`, description: `${result.positions} posiciones actualizaron inventario, kardex, costos y contabilidad.` });
    }
    catch (err) {
      const detail = err instanceof Error ? err.message : "No se pudo contabilizar el cargue inicial";
      setError(detail);
      showToast({ tone: "error", title: "No se pudo contabilizar el cargue inicial", description: detail, retry: () => void post() });
    }
  ...
  return <div className="space-y-4">
    ...
    {confirming && preview ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" onMouseDown={() => !busy && setConfirming(false)}><section className="w-full max-w-lg rounded-lg bg-white p-5 shadow-xl" onMouseDown={(event) => event.stopPropagation()}><h2 className="text-xl font-semibold">Confirmar cargue inicial</h2><p className="mt-2 text-sm text-neutral-600">Se cargarán <strong>{preview.totals.units.toLocaleString("es-CO")} unidades</strong> por <strong>${preview.totals.value.toLocaleString("es-CO")}</strong>. La operación afectará inventario, kardex, costos y contabilidad y no podrá repetirse con el mismo archivo.</p><div className="mt-5 flex justify-end gap-2"><Button disabled={busy} onClick={() => setConfirming(false)} type="button" variant="secondary">Cancelar</Button><Button loading={busy} onClick={() => void post()} type="button">{busy ? "Contabilizando cargue…" : "Sí, contabilizar"}</Button></div></section></div> : null}
  </div>;
}
