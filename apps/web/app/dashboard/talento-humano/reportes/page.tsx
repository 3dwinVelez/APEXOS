  ...
  async function load() {
    ...
    try {
      const [employeeRows, punchRows, activityRows, routeRows] = await Promise.all([
        supabaseFetch<Employee[]>("/rest/v1/employees?select=*&order=first_name.asc"),
        supabaseFetch<Punch[]>(`/rest/v1/time_punches?select=*&punch_date=gte.${from}&punch_date=lte.${to}&order=punched_at.asc`),
        supabaseFetch<Activity[]>(`/rest/v1/gps_pings?select=*&source=eq.work_activity&captured_at=gte.${from}T00:00:00-05:00&captured_at=lte.${to}T23:59:59-05:00&order=captured_at.asc`),
        supabaseFetch<RouteRow[]>(`/rest/v1/operational_routes?select=id,code,vehicle_plate,start_time,end_time,route_date&route_date=gte.${from}&route_date=lte.${to}`)
      ]);
