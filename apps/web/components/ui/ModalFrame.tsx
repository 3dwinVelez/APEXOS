...
export function ModalFrame({ title, children, onClose, maxWidth = "md:max-w-2xl" }: ModalFrameProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end bg-neutral-950/55 p-0 md:items-center md:justify-center md:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section aria-modal="true" aria-labelledby="apex-modal-title" className={`max-h-[calc(100dvh-1rem)] w-full max-w-[100vw] overflow-x-hidden overflow-y-auto rounded-t-md border border-line bg-surface text-content-body shadow-lg md:max-h-[calc(100dvh-3rem)] md:max-w-[calc(100vw-3rem)] md:rounded-md ${maxWidth}`} role="dialog">
