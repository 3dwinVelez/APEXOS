...
Antes de promover `desarrollo -> develop`, ejecutar:

```powershell
npm run qa:preqa:evidence -- <manifiesto-preqa.json>
```

Esta compuerta exige la certificación local versionada, las regresiones aplicables y la autorización explícita para desplegar en QA. El resultado mantiene `qa_status: pending`: la aprobación funcional de QA solo se obtiene después del despliegue y sigue siendo obligatoria para `develop -> main` mediante `qa:approval:evidence`.
